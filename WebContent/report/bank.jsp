...
<body>
	...
	%>
	<div id="all" class="container container-top">
		<table class="table table-bordered table-striped">
			<caption>银行日志报表 <%= type %>（<%= begin %> ~ <%= end %>）</caption>
			<thead>
				<tr>
					<td class="text-center" colspan="7">银行（<%= subordinate.size() %>人）</td>
				</tr>
				<tr>
					<td>银行业务总管：</td>
				...
				<tr>
					<td class="text-center" colspan="7">总结报告</td>
				</tr>
				<tr>
					<td>#</td>
					<td>时间</td>
			...
			<tbody>
				<%
				for (int i = 0, j = 1; i < summary.size(); ++ i) {
					Summary s = summary.get(i);
					if (s.getEmployee().getType() == Employee.Conductor) {
						out.print("<tr>");
						out.print("<td>" + (j ++) + "</td>");
						out.print("<td>" + Util.formatTime(s.getTime()) + "</td>");
						out.print("<td colspan='5'>" + s.getContent() + "</td>");
						out.print("</tr>");
					}
				}
				%>
			...
			<thead>
				<tr>
					<td class="text-center" colspan="7">部门总结报告</td>
				</tr>
				<tr>
					<td>#</td>
					<td>时间</td>
			...
			<tbody>
				...
				for (int i = 0, j = 1; i < summary.size(); ++ i) {
					Summary s = summary.get(i);
					if (s.getEmployee().getType() == Employee.Manager) {
						out.print("<tr>");
						out.print("<td>" + (j ++) + "</td>");
						out.print("<td>" + Util.formatTime(s.getTime()) + "</td>");
						out.print("<td>" + s.getEmployee().getDepartment().getName() + "</td>");
			...
			<thead>
				<tr>
					<td class="text-center" colspan="7">员工总结报告</td>
				</tr>
				<tr>
					<td>#</td>
					<td>时间</td>
			...
			<tbody>
				...
				for (int i = 0; i < summary.size(); ++ i) {
					Summary s = summary.get(i);
					if (s.getEmployee().getType() == Employee.Operator) {
						out.print("<tr>");
						out.print("<td>" + (i + 1) + "</td>");
						out.print("<td>" + Util.formatTime(s.getTime()) + "</td>");
			...
			<thead>
				<tr>
					<td class="text-center" colspan="7">详细日志</td>
				</tr>
				<tr>
					<td>#</td>
					<td>时间</td>
